...
if st.sidebar.button("🌊 執行 LSTM 預測"):
    ...
    if model is None:
        ...
        if model_type == "LSTM":
            NN = tf.keras.layers.LSTM
        elif model_type == "GRU":
            NN = tf.keras.layers.GRU
        elif model_type == "SimpleRNN":
            NN = tf.keras.layers.SimpleRNN
        else:
            st.error(f"未知的模型類型: {model_type}")
            st.stop()

        model = Sequential([
            Input(shape=(look_back, 1)),
            NN(lstm_units, return_sequences=True), Dropout(dropout_rate),
            NN(lstm_units, return_sequences=False), Dropout(dropout_rate),
            Dense(1)
        ])
